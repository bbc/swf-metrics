# swf-metrics [![Build Status](https://travis-ci.org/bbc/swf-metrics.svg?branch=master)](https://travis-ci.org/bbc/swf-metrics)

> Introspective metrics and breakdowns of pending and finished AWS Simple Workflow executions.

`swf-metrics` is a command line tool which will help you answer the following questions about your [AWS Simple Workflow](https://aws.amazon.com/swf/) executions:

* what are the main **failure reasons**?
* which activity **fails the most**?
* what is the **current completion status**?
* what is the **time to complete** of each activity?
...
```bash
$ npm install -g swf-metrics
```

## With Docker

A docker image is automatically built and made available on [Docker Hub](https://hub.docker.com/r/bbcrd/swf-metrics/).

```bash
$ docker pull bbcrd/swf-metrics
```

# Configure

Ensure the following environment variables are set prior to running `swf-metrics`:
...
Options:
  ...
  --workflow-id     Retrieve metrics only for this workflow execution.
                                                                     [string]
  --time-threshold  Hide cell values below this threshold (in seconds).
                                                        [number] [default: 1]
  --since           Position in time from which we start retrieving the
                    history of data.                                 [string]
...
$ docker run --rm -it \
    ...
    bbcrd/swf-metrics <command> --domain <domain> [options]
```
...
║ 27:in `handle'/app/.gems/gems/example_0.0.1/lib/f │     │        │ 4befff3f-8c9b-44e9-95ae-265a124e03b0 ║
║ swf_workflow/worker.rb:29:in `block in start'/app/.gems/ │     │        │ 68ea5d47-bdbe-436f-98f4-e9994247a756 ║
...
║ ems/example_0.0.1/lib/swf_workflow/worker.rb │     │        │                                      ║
